...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        when(lemonadeState){
            SELECT->{
                // 1f the state is SELECT that means on tap we will try to squeeze the Lemon so set the state as SQUEEZE
                lemonadeState= SQUEEZE
                // determining the size of tree by peek()function
                val tree: LemonTree= lemonTree
                lemonSize=tree.pick();
                // 10 starting there is no squeeze so count-B
                squeezeCount=0}

            SQUEEZE -> {
                // now we are squeezing so count will be increased by1and size will be decreased by1
                squeezeCount += 1
                lemonSize -= 1

                // now the lemonade State will in Squeeze state until we do not complete the whole Lemon(LemonSS
                // so if the lemonSize is0,then the state will be DRINK till then we have to Squeeze
                lemonadeState = if (lemonSize == 0) {
                    DRINK
                } else SQUEEZE
            }
// if state is Drink then we will just drink the lemonade and
            DRINK -> {
    ...
    private fun setViewElements() {
        ...
        when(lemonadeState){
            SELECT->{
                textAction.text="Click to selectalemon!"
